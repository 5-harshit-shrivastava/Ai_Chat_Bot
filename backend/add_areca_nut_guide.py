#!/usr/bin/env python3
"""
Script to add areca nut application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_areca_nut_guide_to_database():
    """Add areca nut application guide information to the database"""
    
    areca_nut_guide_content = """How to Use Navyakosh: Application Guide for Nuts & Pulses
...
Active microorganisms in the fertilizer (Mycorrhiza (VAM), Phosphate Solubilizing Bacteria, Azospirillum, Potassium Mobilizing Biofertilizer (KMB), Pseudomonas, etc.) get activated and help plants to get and utilize all macro and micronutrients throughout the crop cycle."""
    ...
    metadata = {
        ...
        "special_instructions": {
            "areca_nut": "dig_ring_around_stem_cover_with_soil"
        },
    ...
    try:
        doc_id = add_document(areca_nut_guide_content, metadata, db_session)
        print(f"✅ Successfully added Areca Nut Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Areca Nut Application Guide to database...")
    result = add_areca_nut_guide_to_database()
    
    if result:
        print("\n🎉 Areca Nut Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for areca nut fertilization.")
    else:
        print("\n💥 Failed to add Areca Nut Application Guide. Please check the error messages above.")