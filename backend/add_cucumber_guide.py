#!/usr/bin/env python3
"""
Script to add cucumber application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_cucumber_guide_to_database():
    """Add cucumber application guide information to the database"""
    
    cucumber_guide_content = """How to Use Navyakosh: Application Guide for Vegetables
...
Note: For Cucumber, application is required two times throughout the crop cycle."""
    ...
    metadata = {
        ...
        "special_instructions": {
            "cucumber": "two_applications_required_throughout_crop_cycle",
            "application_method": "base_application_per_plant"
        },
        "crop_types": ["vegetables", "cucurbits"]
    }
    ...
    try:
        doc_id = add_document(cucumber_guide_content, metadata, db_session)
        print(f"✅ Successfully added Cucumber Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Cucumber Application Guide to database...")
    result = add_cucumber_guide_to_database()
    
    if result:
        print("\n🎉 Cucumber Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for cucumber fertilization.")
    else:
        print("\n💥 Failed to add Cucumber Application Guide. Please check the error messages above.")