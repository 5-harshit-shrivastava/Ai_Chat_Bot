#!/usr/bin/env python3
"""
Script to add mango application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_mango_guide_to_database():
    """Add mango application guide information to the database"""
    
    mango_guide_content = """How to Use Navyakosh: Application Guide for Fruits & Vegetables
...
Key Microorganisms: Mycorrhiza (VAM), Phosphate Solubilizing Bacteria (PSB), Azospirillum, Potassium Mobilizing Biofertilizer (KMB), Pseudomonas, etc."""
    ...
    metadata = {
        ...
        "special_instructions": {
            "mango": "dig_ring_around_stem_cover_with_soil",
    ...
    try:
        doc_id = add_document(mango_guide_content, metadata, db_session)
        print(f"✅ Successfully added Mango Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Mango Application Guide to database...")
    result = add_mango_guide_to_database()
    
    if result:
        print("\n🎉 Mango Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for mango fertilization.")
    else:
        print("\n💥 Failed to add Mango Application Guide. Please check the error messages above.")