#!/usr/bin/env python3
"""
Script to add mustard application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_mustard_guide_to_database():
    """Add mustard application guide information to the database"""
    
    mustard_guide_content = """How to Use Navyakosh: Application Guide for Grains & Millets
...
Key Microorganisms: Azotobacter, Phosphate Solubilizing Bacteria (PSB), and Sulphur Solubilizing Bacteria (SSB)."""
    ...
    metadata = {
        ...
        "microorganisms": ["Azotobacter", "Phosphate Solubilizing Bacteria (PSB)", "Sulphur Solubilizing Bacteria (SSB)"],
        "special_instructions": {
            "mustard": "broadcast_pattern_land_preparation_then_post_weeding",
            "irrigated_crops": "second_dose_after_weeding_for_irrigated_crops"
        },
        "crop_types": ["oil_seeds", "brassicas"]
    }
    ...
    try:
        doc_id = add_document(mustard_guide_content, metadata, db_session)
        print(f"✅ Successfully added Mustard Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Mustard Application Guide to database...")
    result = add_mustard_guide_to_database()
    
    if result:
        print("\n🎉 Mustard Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for mustard fertilization.")
    else:
        print("\n💥 Failed to add Mustard Application Guide. Please check the error messages above.")