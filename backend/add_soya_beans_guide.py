#!/usr/bin/env python3
"""
Script to add soya beans application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_soya_beans_guide_to_database():
    """Add soya beans application guide information to the database"""
    
    soya_beans_guide_content = """How to Use Navyakosh: Application Guide for Nuts & Pulses
...
Active microorganisms in the fertilizer (Rhizobium, Phosphate Solubilizing Bacteria, Vesicular Arbuscular Mycorrhiza (VAM), etc.) get activated and help plants to get and utilize all macro and micronutrients throughout the crop cycle."""
    ...
    metadata = {
        ...
        "crops_covered": ["soya_beans", "soybean"],
        "application_methods": {
        ...
        "special_instructions": {
            "soya_beans": "broadcast_pattern_land_preparation_then_post_weeding",
    ...
    try:
        doc_id = add_document(soya_beans_guide_content, metadata, db_session)
        print(f"✅ Successfully added Soya Beans Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Soya Beans Application Guide to database...")
    result = add_soya_beans_guide_to_database()
    
    if result:
        print("\n🎉 Soya Beans Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for soya beans fertilization.")
    else:
        print("\n💥 Failed to add Soya Beans Application Guide. Please check the error messages above.")