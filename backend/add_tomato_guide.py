#!/usr/bin/env python3
"""
Script to add tomato application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_tomato_guide_to_database():
    """Add tomato application guide information to the database"""
    
    tomato_guide_content = """How to Use Navyakosh: Application Guide for Fruits & Vegetables
...
Key Microorganisms: Mycorrhiza (VAM), Phosphate Solubilizing Bacteria (PSB), Azotobacter, Potassium Mobilizing Biofertilizer (KMB), etc."""
    ...
    metadata = {
        ...
        "microorganisms": ["Mycorrhiza (VAM)", "Phosphate Solubilizing Bacteria (PSB)", "Azotobacter", "Potassium Mobilizing Biofertilizer (KMB)"],
        "special_instructions": {
            "tomato": "two_applications_required_throughout_crop_cycle",
            "application_method": "base_application_per_plant"
        },
        "crop_types": ["vegetables", "fruits", "solanaceae"]
    }
    ...
    try:
        doc_id = add_document(tomato_guide_content, metadata, db_session)
        print(f"✅ Successfully added Tomato Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Tomato Application Guide to database...")
    result = add_tomato_guide_to_database()
    
    if result:
        print("\n🎉 Tomato Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for tomato fertilization.")
    else:
        print("\n💥 Failed to add Tomato Application Guide. Please check the error messages above.")