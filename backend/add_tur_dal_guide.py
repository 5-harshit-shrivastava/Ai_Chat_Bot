#!/usr/bin/env python3
"""
Script to add tur dal (arhar/pigeon pea) application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_tur_dal_guide_to_database():
    """Add tur dal application guide information to the database"""
    
    tur_dal_guide_content = """How to Use Navyakosh: Application Guide for Nuts & Pulses
...
Active microorganisms in the fertilizer (Rhizobium, Phosphate Solubilizing Bacteria, Vesicular Arbuscular Mycorrhiza (VAM), etc.) get activated and help plants to get and utilize all macro and micronutrients throughout the crop cycle."""
    ...
    metadata = {
        ...
        "crops_covered": ["tur_dal", "arhar", "pigeon_pea"],
        "application_methods": {
        ...
        "special_instructions": {
            "tur_dal": "broadcast_pattern_land_preparation_then_post_weeding",
    ...
    try:
        doc_id = add_document(tur_dal_guide_content, metadata, db_session)
        print(f"✅ Successfully added Tur Dal (Arhar/Pigeon Pea) Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Tur Dal (Arhar/Pigeon Pea) Application Guide to database...")
    result = add_tur_dal_guide_to_database()
    
    if result:
        print("\n🎉 Tur Dal (Arhar/Pigeon Pea) Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for tur dal/arhar/pigeon pea fertilization.")
    else:
        print("\n💥 Failed to add Tur Dal (Arhar/Pigeon Pea) Application Guide. Please check the error messages above.")