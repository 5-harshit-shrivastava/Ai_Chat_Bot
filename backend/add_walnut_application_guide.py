#!/usr/bin/env python3
"""
Script to add separate walnut application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_walnut_application_guide_to_database():
    """Add walnut application guide to the database"""
    
    walnut_guide_content = """How to Use Navyakosh: Application Guide for Other Crops
...
Active microorganisms in the fertilizer (Mycorrhiza (VAM), Phosphate Solubilizing Bacteria, Azospirillum, Potassium Mobilizing Biofertilizer (KMB), Pseudomonas, etc.) get activated and help plants to get and utilize all macro and micronutrients throughout the crop cycle."""
    ...
    try:
        doc_id = add_document(walnut_guide_content, metadata, db_session)
        print(f"✅ Successfully added Walnut Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Walnut Application Guide to database...")
    result = add_walnut_application_guide_to_database()
    
    if result:
        print("\n🎉 Walnut Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed walnut-specific application instructions.")
    else:
        print("\n💥 Failed to add Walnut Application Guide. Please check the error messages above.")