#!/usr/bin/env python3
"""
Script to add wheat application guide to the RAG chatbot database.
"""

from db import get_db, init_db
from ingest import add_document

def add_wheat_guide_to_database():
    """Add wheat application guide information to the database"""
    
    wheat_guide_content = """How to Use Navyakosh: Application Guide for Grains & Millets
...
Key Microorganisms: Azospirillium, Azotobacter, Phosphate Solubilizing Bacteria (PSB), and Sulphur Solubilizing Bacteria (SSB)."""
    ...
    metadata = {
        ...
        "microorganisms": ["Azospirillium", "Azotobacter", "Phosphate Solubilizing Bacteria (PSB)", "Sulphur Solubilizing Bacteria (SSB)"],
        "special_instructions": {
            "wheat": "broadcast_pattern_land_preparation_then_post_weeding",
            "irrigated_crops": "second_dose_after_weeding_for_irrigated_crops"
        },
        "crop_types": ["grains", "cereals"]
    }
    ...
    try:
        doc_id = add_document(wheat_guide_content, metadata, db_session)
        print(f"✅ Successfully added Wheat Application Guide to database!")
...
if __name__ == "__main__":
    print("Adding Wheat Application Guide to database...")
    result = add_wheat_guide_to_database()
    
    if result:
        print("\n🎉 Wheat Application Guide has been successfully added to the RAG chatbot database!")
        print("The chatbot can now provide detailed application instructions for wheat fertilization.")
    else:
        print("\n💥 Failed to add Wheat Application Guide. Please check the error messages above.")